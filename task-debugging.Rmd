...
library("shape")
plot_fib <- function(x) {
  plot(-max(x):max(x), -max(x):max(x), type = "n", 
       xlab = "", ylab = "", main = "Fibonacchi spiral") #empty plot
    x.coor <- c(0, 0, 
                cumsum(x * rep(c(1, 0, -1, 0),
                               length = length(x)))) #x-coordinate for the center of the circle
     y.coor <- c(0, 0,
                cumsum(x * rep(c(0, 1, 0, -2),
                               length=length(x)))) #y-coordinate for the center of the circle
